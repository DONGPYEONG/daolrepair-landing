#!/usr/bin/env python3
"""Node.js 스크립트를 Python으로 변환 — FAQ 통합 페이지 생성"""
import json, os

BASE = r"c:\Users\다올리페어\Downloads\landingpage-20260326T082547Z-3-001\landingpage"

with open(os.path.join(BASE, ".tmp", "all_faqs.json"), "r", encoding="utf-8") as f:
    data = json.load(f)
...
for d in data:
    c = d["category"]
    cat_counts[c] = cat_counts.get(c, 0) + len(d["faqs"])
    total_faqs += len(d["faqs"])

# Compact data
compact_data = []
for d in data:
    compact_data.append([d["file"], d["title"], d["category"], [[fq["q"], fq["a"]] for fq in d["faqs"]]])

# Pick 20 diverse structured data questions
cat_targets = {"iphone": 5, "watch": 3, "guide": 4, "macbook": 3, "ipad": 2, "airpods": 2, "pencil": 1}
sd_picks = []
cat_picked = {}
for d in data:
    cat = d["category"]
    cat_picked.setdefault(cat, 0)
    target = cat_targets.get(cat, 1)
    if cat_picked[cat] < target and d["faqs"]:
        sd_picks.append({"q": d["faqs"][0]["q"], "a": d["faqs"][0]["a"]})
        cat_picked[cat] += 1
    if len(sd_picks) >= 20:
        break
...
sd_json = json.dumps({
    ...
    "@type": "FAQPage",
    "mainEntity": [{"@type": "Question", "name": p["q"], "acceptedAnswer": {"@type": "Answer", "text": p["a"]}} for p in sd_picks]
}, ensure_ascii=False)

cat_labels = {"all": "전체", "iphone": "아이폰", "watch": "애플워치", "guide": "수리 가이드", "macbook": "맥북", "ipad": "아이패드", "airpods": "에어팟", "pencil": "애플펜슬"}
cat_order = ["all", "iphone", "watch", "guide", "macbook", "ipad", "airpods", "pencil"]

tabs_lines = []
for cat in cat_order:
    count = total_faqs if cat == "all" else cat_counts.get(cat, 0)
    label = cat_labels[cat]
    active = " active" if cat == "all" else ""
    tabs_lines.append(f'      <button class="faq-tab{active}" data-cat="{cat}">{label} <span class="faq-tab-count">{count}</span></button>')
tabs_html = "\n".join(tabs_lines)

compact_json = json.dumps(compact_data, ensure_ascii=False)

# Read the mjs template structure and reproduce in Python
html = f'''<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
  ...
  <script type="application/ld+json">{sd_json}</script>
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .art-nav-reserve-btn {{ background: #E8732A; color: #fff; padding: 6px 13px; border-radius: 20px; font-size: 12.5px; font-weight: 700; border: none; cursor: pointer; transition: background 0.2s; white-space: nowrap; font-family: var(--font); }}
    ...
    .faq-search-input {{ width: 100%; padding: 16px 48px 16px 48px; border: 2px solid var(--border); border-radius: 14px; font-size: 16px; font-family: var(--font); outline: none; transition: border-color 0.2s, box-shadow 0.2s; background: #fafafa; color: var(--dark); }}
    ...
    .faq-search-clear {{ position: absolute; right: 14px; top: 50%; transform: translateY(-50%); background: #e8e8e8; border: none; font-size: 14px; line-height: 1; color: #888; cursor: pointer; padding: 4px 8px; border-radius: 6px; display: none; font-family: var(--font); font-weight: 600; }}
    .faq-search-clear:hover {{ background: #ddd; color: var(--dark); }}
    .faq-result-info {{ font-size: 13px; color: var(--muted); margin-bottom: 18px; min-height: 20px; }}
    .faq-result-info strong {{ color: var(--orange); font-weight: 800; }}
    .faq-tabs {{ display: flex; gap: 8px; margin-bottom: 28px; flex-wrap: wrap; position: sticky; top: 64px; z-index: 100; background: #fff; padding: 12px 0; border-bottom: 1px solid var(--border); }}
    .faq-tab {{ display: flex; align-items: center; gap: 6px; padding: 8px 14px; border-radius: 50px; border: 1.5px solid var(--border); background: #fff; font-size: 13px; font-weight: 700; color: var(--muted); cursor: pointer; transition: all 0.18s; font-family: var(--font); white-space: nowrap; }}
    .faq-tab:hover {{ border-color: var(--orange); color: var(--orange); }}
    .faq-tab.active {{ background: var(--dark); border-color: var(--dark); color: #fff; }}
    .faq-tab-count {{ font-size: 11px; font-weight: 800; padding: 1px 6px; border-radius: 10px; }}
    ...
    .faq-group-title {{ font-size: 13px; font-weight: 700; color: var(--muted); padding: 10px 0 8px; border-bottom: 1px solid #f0f0f0; margin-bottom: 4px; display: flex; align-items: center; gap: 8px; }}
    .faq-group-title::before {{ content: ''; display: inline-block; width: 3px; height: 14px; background: var(--orange); border-radius: 2px; flex-shrink: 0; }}
    .faq-item {{ border-bottom: 1px solid #f5f5f5; }}
    .faq-q {{ display: flex; align-items: flex-start; gap: 10px; padding: 16px 4px; cursor: pointer; width: 100%; background: none; border: none; text-align: left; font-family: var(--font); font-size: 15px; font-weight: 700; color: var(--dark); line-height: 1.55; transition: color 0.15s; }}
    .faq-q:hover {{ color: var(--orange); }}
    .faq-q-label {{ flex-shrink: 0; font-size: 13px; font-weight: 900; color: var(--orange); margin-top: 1px; min-width: 22px; }}
    .faq-q-arrow {{ flex-shrink: 0; margin-left: auto; margin-top: 3px; width: 16px; height: 16px; color: #ccc; transition: transform 0.25s ease, color 0.15s; }}
    ...
    .faq-a {{ padding: 0 4px 16px 36px; font-size: 14px; line-height: 1.75; color: #444; }}
    .faq-a-link {{ display: inline-block; margin-top: 10px; font-size: 12px; color: var(--orange); text-decoration: none; font-weight: 600; transition: opacity 0.15s; }}
    ...
    .faq-top-btn {{ position: fixed; bottom: 24px; right: 24px; width: 44px; height: 44px; border-radius: 50%; background: var(--orange); color: #fff; border: none; cursor: pointer; font-size: 20px; display: none; align-items: center; justify-content: center; box-shadow: 0 4px 16px rgba(232,115,42,0.3); transition: opacity 0.2s, transform 0.2s; z-index: 500; }}
    .faq-top-btn.show {{ display: flex; }}
    .faq-top-btn:hover {{ transform: scale(1.08); }}
    @media (max-width: 600px) {{ .faq-tabs {{ gap: 6px; }} .faq-tab {{ padding: 7px 11px; font-size: 12px; }} .faq-q {{ font-size: 14px; padding: 14px 2px; }} .faq-a {{ padding-left: 32px; font-size: 13px; }} }}
  </style>
</head>
<body>
  ...
  function render() {{
    ...
    for (var i = 0; i < articles.length; i++) {{
      ...
      for (var j = 0; j < faqs.length; j++) {{
        ...
        groupItems += '<div class="faq-item"><button class="faq-q" aria-expanded="false"><span class="faq-q-label">Q</span><span>' + qDisp + '</span><svg class="faq-q-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button><div class="faq-a-wrap"><div class="faq-a">' + aDisp + '<br><a class="faq-a-link" href="' + escHtml(file) + '">원문 보기 \\u2192</a></div></div></div>';
      }}
      if (groupCount > 0) {{
        html += '<div class="faq-group"><div class="faq-group-title">' + escHtml(title) + '</div>' + groupItems + '</div>';
      }}
    }}
...
out_path = os.path.join(BASE, "articles", "faq.html")
with open(out_path, "w", encoding="utf-8") as f:
    f.write(html)

print(f"FAQ 페이지 생성 완료: {total_faqs}개 FAQ, {len(data)}개 글")
print(f"파일: {out_path}")
print(f"파일 크기: {os.path.getsize(out_path) // 1024}KB")
