...
# 액정·배터리 모두 정품만 사용 (OEM·fog·호환 X)
COMMON_IPAD_PARTS = [
    "디스플레이(액정) — 정품 추출 또는 재생 액정만 사용 (OEM·호환 X)",
    "배터리 — 정품 부품만 사용 (fog·OEM 사용 안 함)",
...
def build_html(slug: str, data: dict) -> str:
    ...
    if device == "아이폰":
        ...
        parts_summary = "다올리페어 아이패드는 액정·배터리 모두 정품 부품만 사용합니다 (OEM·fog·호환 사용 안 함)."
    ...
    if device == "아이폰":
        ...
        daolrepair_options = (
            f"{model}은 다올리페어에서 정품 부품만 사용합니다. "
            "<strong>액정</strong>은 정품 추출 또는 재생 액정만 사용 (OEM·호환 X), "
            "<strong>배터리</strong>도 정품 부품만 사용합니다 (fog·OEM 사용 안 함, 패드는 셀 교체 옵션 없음). "
            "부품별 가격을 공개하고 비교 견적을 안내합니다. 90일 보증, 수리 실패 시 비용 0원, 당일 처리 가능합니다."
        )
