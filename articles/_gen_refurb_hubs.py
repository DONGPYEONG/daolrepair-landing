...
REFURB_HUBS = {
    "iphone-refurbish-guide": {
        ...
        "faqs": [
            ...
            ("사설 수리는 보증이 어떻게 되나요?",
             "사설 수리점에 따라 다르지만, 다올리페어는 90일 보증을 표준으로 적용하고 부품 옵션(액정: 정품·DD, 배터리: 셀 교체·일반·정품 인증)별로 가격을 모두 공개합니다. 수리 실패 시 비용 0원 정책도 있어 부담 없이 견적부터 받아보실 수 있습니다."),
    ...
    "ipad-refurbish-guide": {
        ...
        "faqs": [
            ...
            ("아이패드 화면 단독 교체가 정말 가능한가요?",
             "공식센터는 부품 단독 수리가 제한적이지만, 다올리페어 같은 사설 수리점은 액정 단독 교체가 가능합니다. 다올리페어는 아이패드 액정에 정품 추출 또는 재생 액정만 사용합니다 (OEM·호환 사용 안 함)."),
...
{case_no_items}
    ...
    <p>다올리페어는 부품별 옵션을 모두 공개합니다. <strong>아이폰 액정</strong>은 정품·DD 2가지, <strong>아이폰 배터리</strong>는 셀 교체·일반·정품 인증 3옵션. <strong>애플워치 액정</strong>은 정품 단독, <strong>아이패드 액정·배터리</strong>는 정품 부품만 사용합니다. 90일 보증, 수리 실패 시 비용 0원, 당일 처리 가능. 가산·신림·목동 직영점 + 전국 택배 수리.</p>
