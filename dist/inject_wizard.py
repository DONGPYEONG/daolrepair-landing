...
<!-- ── WIZARD MODAL CSS ── -->
<style>
  :root { --orange:#E8732A; --orange-dark:#C55E1A; --dark:#1D1D1F; --white:#FFFFFF; }
  ...
  .wiz-modal-overlay.open { display:flex; animation:fadeIn 0.25s ease; }
  .wiz-modal {
    position:relative; background:#1D1D1F; border-radius:24px;
    width:100%; max-width:680px; max-height:92dvh; overflow-y:auto;
    padding:52px 40px 36px; box-shadow:0 24px 80px rgba(0,0,0,0.6);
    animation:slideUp 0.3s ease;
    font-family:-apple-system,BlinkMacSystemFont,'Apple SD Gothic Neo','Noto Sans KR',sans-serif;
  }
  @media(max-width:600px){
    .wiz-modal{padding:48px 18px calc(24px + env(safe-area-inset-bottom));border-radius:18px;max-height:96dvh;}
  }
  .wiz-modal-close {
  ...
  .wiz-panel.active{display:block;animation:slideUp 0.35s ease;}
  ...
  @media(max-width:600px){
    .wiz-device-grid{gap:8px;}
    .wiz-device-btn{padding:16px 8px 12px;}
    .wiz-device-btn-icon{height:50px;margin-bottom:7px;}
    .wiz-device-btn-icon svg{height:44px;}
    .wiz-device-btn-label{font-size:12px;}
  }
  ...
  .wiz-success-state.show{display:block;animation:slideUp 0.4s ease;}
...
window.artWizOpen = function(courier){
  ...
  if(courier){
    // pre-select 택배 when user returns to step 4
    window._artWizCourier = true;
  } else {
    window._artWizCourier = false;
  }
};
...
def process_file(filepath):
    ...
    if 'artWizOverlay' in content:
        return False

    # 1. Inject wizard CSS before </head>
    if '</head>' in content:
        content = content.replace('</head>', WIZARD_CSS + '</head>', 1)

    # 2. Replace CTA buttons
    # "무료 견적 받기" link → button
    content = re.sub(
        r'<a\s+href="[^"]*\?wizard=1[^"]*"\s+class="art-cta-btn">([^<]+)</a>',
        r'<a href="javascript:void(0)" onclick="artWizOpen(false)" class="art-cta-btn">\1</a>',
        content
    )
    # "택배 수리 접수" link → button (opens wizard with courier pre-note)
    content = re.sub(
        r'<a\s+href="[^"]*#courier[^"]*"\s+class="art-cta-btn-ghost">([^<]+)</a>',
        r'<a href="javascript:void(0)" onclick="artWizOpen(true)" class="art-cta-btn-ghost">\1</a>',
        content
    )

    # 3. Inject wizard HTML + JS before </body>
    content = content.replace('</body>', WIZARD_HTML + WIZARD_JS + '</body>', 1)
...
for fname in os.listdir(ARTICLES_DIR):
    if fname.endswith('.html') and fname != 'index.html':
        fp = os.path.join(ARTICLES_DIR, fname)
...
print(f"완료: {count}개 파일 수정, {skipped}개 건너뜀 (이미 적용됨)")
