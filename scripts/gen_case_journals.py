    ...
    <div class="wiz-panel" id="awp4">
      ...
      <div class="wiz-form">
        ...
        <div style="margin-bottom:12px">
          <label class="wiz-form-label">제품 잠금 해제 비밀번호 <span style="color:#FF6B66;">*</span></label>
          <div style="display:flex;gap:8px;margin-bottom:8px;">
            <button type="button" id="artPwdHave" onclick="artWizPwdChoose('have')" style="flex:1;background:rgba(255,255,255,0.04);border:1.5px solid rgba(255,255,255,0.15);border-radius:10px;padding:10px;color:#fff;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">있음 (입력)</button>
            <button type="button" id="artPwdNone" onclick="artWizPwdChoose('none')" style="flex:1;background:rgba(255,255,255,0.04);border:1.5px solid rgba(255,255,255,0.15);border-radius:10px;padding:10px;color:#fff;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">없음 (초기화 후 발송)</button>
          </div>
          <input class="wiz-fi" type="text" id="artWPwd" placeholder="잠금 해제 비밀번호 (숫자 4~6자리)" style="display:none;" maxlength="20">
          <p style="font-size:11.5px;color:rgba(255,255,255,0.55);line-height:1.65;margin-top:8px;">
            ※ 기능 검수를 위해 제품 잠금 해제 비밀번호가 필요합니다.<br>
            ※ 불편하신 경우, 초기화 후 비밀번호 없이 초기 화면 상태로 보내주셔도 됩니다.<br>
            ※ <strong style="color:#FFB87A;">애플워치를 초기화하실 경우 — 초기화 후 반드시 페어링된 아이폰과 함께 보내주세요. 아니면 반송됩니다.</strong>
          </p>
        </div>
...
window.artWizPwdChoose = function(choice){
  window._artPwdChoice = choice;
  var have = document.getElementById('artPwdHave');
  var none = document.getElementById('artPwdNone');
  var input = document.getElementById('artWPwd');
  function setActive(btn, on){
    btn.style.background = on ? 'rgba(232,115,42,0.18)' : 'rgba(255,255,255,0.04)';
    btn.style.borderColor = on ? '#E8732A' : 'rgba(255,255,255,0.15)';
    btn.style.color = on ? '#E8732A' : '#fff';
  }
  setActive(have, choice==='have');
  setActive(none, choice==='none');
  if(choice==='have'){ input.style.display='block'; setTimeout(function(){input.focus();},100); }
...
window.artWizSubmit = function(){
  ...
  if(!window._artPwdChoice){alert('제품 잠금 해제 비밀번호 유무를 선택해주세요.');return;}
  var pwd='';
  if(window._artPwdChoice==='have'){
    pwd=document.getElementById('artWPwd').value.trim();
    if(!pwd){alert('잠금 해제 비밀번호를 입력해주세요. 없으시면 "없음 (초기화 후 발송)"을 선택해주세요.');return;}
  }
  var memo = document.getElementById('artWMemo').value.trim();
  var pwdLine = (window._artPwdChoice==='have') ? '🔑 잠금 해제 비밀번호: '+pwd : '🔓 비밀번호 없음 — 초기화 후 발송 예정';
  memo = pwdLine + (memo ? '\n'+memo : '');
  if(artPhotos.length>0) memo = '📷 사진 '+artPhotos.length+'장 첨부됨\n'+memo;
  var d = ART_DEVICES.find(function(x){return x.key===artSel.device;});
  try{fetch(APPS_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain'},body:JSON.stringify({type:'estimate',device:d?d.label:artSel.device,model:artSel.model,repairs:artSel.repairs.join(', '),name:name,phone:phone,method:method,pwd_choice:window._artPwdChoice,pwd:pwd,memo:memo,photos:artPhotos})}).catch(function(){});}catch(e){}
...
window.artWizReset = function(){
  ...
  document.getElementById('artWPwd').value='';
  document.getElementById('artWPwd').style.display='none';
  window._artPwdChoice=null;
  ['artPwdHave','artPwdNone'].forEach(function(id){var b=document.getElementById(id);if(b){b.style.background='rgba(255,255,255,0.04)';b.style.borderColor='rgba(255,255,255,0.15)';b.style.color='#fff';}});
